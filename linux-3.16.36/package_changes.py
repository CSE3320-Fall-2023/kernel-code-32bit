...
def create_tarball():
	...
	modified_files_result = subprocess.check_output("diff -pqr ../linux-3.16.36/ ../.backup_do_not_remove/ 2>/dev/null|grep differ |awk -e \'{print $2}\'", shell=True);
	...
	print "File "+file_name + " is ready to upload to Blackboard."
...
def main(argv):
	...
	try:
		...
		if len(opts) == 0:
			print 'package_changes.py --assignment <assignment> --user <your omega.uta.edu username>'
                	sys.exit(2)
